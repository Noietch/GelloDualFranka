...
def _read_arm_ctrl(name, readers) -> np.ndarray:
    """This arm's 8-vec [j1..j7, grip(0-1)]. THE single calibration path shared by
    teleop-sim and teleop-robot, so MuJoCo and the real robot are driven by the
    exact same numbers (calib_sim + sim_offsets/sim_signs). An uncontrolled arm
    (port=none) holds the FR3 home pose."""
    cfg = G.ARMS[name]
    reader = readers[name]
    if reader is None:
    ...
    try:
        with mujoco.viewer.launch_passive(sim.model, sim.data) as viewer:
            while viewer.is_running():
                t0 = time.time()
                sim.set_command({
                    "left": _read_arm_ctrl("left", readers),
                    "right": _read_arm_ctrl("right", readers),
                })
...
def teleop_robot() -> None:
    """GELLO -> MuJoCo viewer AND real robot, driven by the SAME numbers.

    Identical to teleop-sim's read/calibration path (_read_arm_ctrl), plus it
    publishes that same per-arm command to ROS2 (gello/joint_states + gripper)
    for the official C++ joint-impedance controller. The MuJoCo window shows
    exactly what is being sent to the robot. Needs system ROS2 + franka_ros2.
    """
    _check_mjpython()
    import mujoco
    import mujoco.viewer
    ...
    class Publisher(Node):
        ...
        def publish(self, name, vec):
            """vec = [j1..j7, grip(0-1)] — the same vector fed to MuJoCo."""
            js = JointState()
            js.header.stamp = self.get_clock().now().to_msg()
            js.header.frame_id = "fr3_link0"
            js.name = G.FR3_JOINT_NAMES
            js.position = vec[:7].tolist()
    ...
    rclpy.init()
    node = Publisher()
    print("teleop-robot 🚀  (MuJoCo mirror + real publish; close viewer or Ctrl-C)")
    period = 1.0 / G.HZ_SIM
    n_steps = max(1, round(period / sim.model.opt.timestep))
    try:
        with mujoco.viewer.launch_passive(sim.model, sim.data) as viewer:
            while viewer.is_running():
                t0 = time.time()
                cmds = {name: _read_arm_ctrl(name, readers) for name in G.ARMS}
                sim.set_command(cmds)
                for name in G.ARMS:
                    if readers[name] is not None:
                        node.publish(name, cmds[name])
                rclpy.spin_once(node, timeout_sec=0.0)
                for _ in range(n_steps):
                    mujoco.mj_step(sim.model, sim.data)
                viewer.sync()
